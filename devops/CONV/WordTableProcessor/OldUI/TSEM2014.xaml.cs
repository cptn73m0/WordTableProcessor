...
{
    public partial class TSEM2014 : UserControl
    {
                    ...
                    using (WordprocessingDocument doc = WordprocessingDocument.Open(wordFilePath!, true))
                    {
                        ...
                        foreach (var row in rows)
                        {
                            ...
                            if (cells.Count > 1)
                            {
                                ...
                                if (Regex.IsMatch(code, @"^\d{6}.*$")) // Шаблон ######*
                                {
                                    if (cells.Count > 6)
                                    {
                ...
                using (var reader = new StreamReader(csvFilePath, encoding))
                {
                    while (!reader.EndOfStream)
                    {
                        ...
                        if (line != null)
                        {
                            ...
                            {
                                csvData.Add(cells);
                                logBuilder.AppendLine($"[{DateTime.Now}] CSV строка: Код={cells[1].Trim()}, Таблица={cells[0].Trim()}");
                            }
                        }
                    }
                }
                        ...
                        foreach (int t in tableIndices)
                        {
                            ...
                            foreach (var row in rows)
                            {
                                ...
                                if (cells.Count > 1)
                                {
                                    ...
                                    {
                                        bool found = false;
                                        string targetTableNum = t == 1 ? "1" : "2";
                                        for (int csvIndex = 0; csvIndex < csvData.Count; csvIndex++)
                                        {
                                            string csvCode = csvData[csvIndex][1].Trim();
                                            string csvTableNum = csvData[csvIndex][0].Trim();
                                            if (code.StartsWith(csvCode) && csvTableNum == targetTableNum)
                                            {
                                                found = true;
                                                if (cells.Count > 6)
                                                {
                                                    cells[4].RemoveAllChildren(); // 5-й столбец (PRICE)
                                                    cells[4].Append(new Paragraph(new Run(new Text(FormatNumber(csvData[csvIndex][4])) { Space = SpaceProcessingModeValues.Preserve })));
                                                    cells[5].RemoveAllChildren(); // 6-й столбец (OTM)
                                                    cells[5].Append(new Paragraph(new Run(new Text(FormatNumber(csvData[csvIndex][5])) { Space = SpaceProcessingModeValues.Preserve })));
                                                    cells[6].RemoveAllChildren(); // 7-й столбец (INDX)
                                                    cells[6].Append(new Paragraph(new Run(new Text(FormatNumber(csvData[csvIndex][6])) { Space = SpaceProcessingModeValues.Preserve })));
                                                    updatedRows++;
                                                }
                                                break;
                                            }
        ...
        private string FormatNumber(string input)
        {
            ...
            // Удаляем пробелы, неразрывные пробелы и BOM
            string cleanedInput = input.Trim().Replace(" ", "").Replace("\u00A0", "").Replace("\uFEFF", "");
            try
            {
            ...
            catch (FormatException ex)
            {
                ...
                Dispatcher.InvokeAsync(async () =>
                {
                    ...
                    await File.AppendAllTextAsync(LogFilePath, $"[{DateTime.Now}] Ошибка формата числа: '{input}' (очищено: '{cleanedInput}', ASCII: {asciiDebug}). Детали: {ex.Message}\n");
                });
                return "0,00";
            }
        }
