...
{
    public partial class FER2020_0 : UserControl
    {
        ...
        private void SelectWordButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == true)
            {
                ...
                StatusTextBox.Text += $"Выбран документ Word: {wordFilePath}\n";
            }
        }
        ...
        private async void CleanTableButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("Сначала выберите документ Word!", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            ...
            try
            {
                await Task.Run(() =>
                {
                    using (WordprocessingDocument doc = WordprocessingDocument.Open(wordFilePath, true))
                    {
                        ...
                        var table = tables[3]; // Таблица 4 (индекс начинается с 0)
                        ...
                        foreach (var row in rows)
                        {
                            ...
                            if (cells.Count > 0)
                            {
                                string code = cells[0].InnerText.Trim(); // 1-я колонка
        ...
        private async void UpdatePricesButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("Выберите оба файла: Word и CSV!", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    using (WordprocessingDocument doc = WordprocessingDocument.Open(wordFilePath, true))
                    {
                        ...
                        for (int t = 0; t < 4; t++) // Таблицы 1-4 (индекс начинается с 0)
                        {
                            var table = tables[t];
                            var rows = table.Elements<TableRow>().ToList();
                            foreach (var row in rows)
                            {
                                ...
                                if (cells.Count > 0)
                                {
                                    ...
                                    if (Regex.IsMatch(code, @"^\d{2}-\d{2}-\d{3}-\d{2}.*$"))
                                    {
                                        ...
                                        if (!found)
                                        {
                                            Dispatcher.Invoke(() => StatusTextBox.Text += $"Код {code} не найден в CSV для таблицы {t + 1}\n");
                                        }
                                    }
                                }
                                processedRows++;
